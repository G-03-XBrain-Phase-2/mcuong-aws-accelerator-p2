...
> [!NOTE] 💡
> Tài liệu này được tổng hợp và cập nhật dựa trên lộ trình HashiCorp Certified: Terraform Associate 2026 (Udemy). Tập trung vào các Module 31, 33, 34, 35 và 36.
...
### Quá trình làm việc của Terraform khi chạy `terraform plan/apply`<u>:</u>
...
- **Bước 3 (Reconcile):** Lên kế hoạch (`Plan`):
  - Nếu code `.tf` khai báo thêm tài nguyên mới -&gt; **Terraform sẽ Tạo mới (Create).**
  - Nếu code `.tf` có thay đổi thông số-&gt; Terraform sẽ **Cập nhật (Update)** hoặc **Tái tạo (Replace)**.
  - Nếu code `.tf` xóa bỏ dòng tài nguyên cũ -&gt; Terraform sẽ **Hủy bỏ (Delete/Destroy)**.
