...
- **Cách tham chiếu:** Cú pháp để trỏ tới giá trị của tài nguyên này từ nơi khác trong code là: `<RESOURCE TYPE>.<RESOURCE NAME>.<ATTRIBUTE>`*(Ví dụ:* `aws_instance.my_web.public_ip`*)*.
...
- **Vòng lặp For trong Expression:**
  - *Ví dụ:* Trích xuất tất cả ID của instance ra một list: `value = [for instance in aws_instance.web : instance.id]`
...
- **Cấu trúc gọi module:**
  ...
  ```
- **Lấy dữ liệu ra từ module:** Sử dụng cú pháp `module.<MODULE_NAME>.<OUTPUT_NAME>`*(Ví dụ:* `vpc_id = module.vpc.vpc_id`*)*.
